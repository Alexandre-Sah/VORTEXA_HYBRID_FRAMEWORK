...
document.addEventListener('DOMContentLoaded', () => {
    const mainContentDiv = document.getElementById('main-content'); // Alterado de 'mainContent' para 'mainContentDiv' e ID para 'main-content'
    const sidebarList = document.getElementById('sidebar-list');
    const langButtons = document.querySelectorAll('.lang-button'); // Alterado para classe
    const darkModeToggle = document.getElementById('theme-toggle'); // Alterado ID
    ...
    const content = {
        'pt': {
                ...
                sections: [
                    {
                        ...
                        content: `
                            ...
                            <p>Mais do que um veículo de corrida, o VORTEXA HYBRID é uma plataforma de pesquisa e desenvolvimento, um laboratório sobre rodas que avança as fronteiras da eletrificação, IA embarcada, segurança funcional e resiliência em am...</p>
                        `
                    },
                    {
                        ...
                        content: `
                            <ul>
                                <li><strong>Powertrain Híbrido a Hidrogênio:</strong> Combinação de motor de combustão interna (ICE) otimizado para H2 e motores elétricos de alta performance.</li>
                                <li><strong>IA Embarcada (Vortexa Brain™):</strong> Sistema de inteligência artificial com capacidades de aprendizado adaptativo e tomada de decisão em tempo real.</li>
                                <li><strong>Sistema de Sobrevivência (Survivor Capsule™):</strong> Proteção avançada e resiliência estrutural para ambientes extremos.</li>
                                <li><strong>Modularidade e Escalabilidade:</strong> Arquitetura que permite reconfiguração rápida para diferentes missões e cenários.</li>
                            </ul>
                        `
                    },
                    {
                        ...
                        subtitle: 'Aplicações',
                        content: `
                            <p>O VORTEXA HYBRID é versátil e pode ser empregado em diversas áreas, incluindo:</p>
                            <ul>
                                <li><strong>World Rally-Raid Championship (W2RC):</strong> Competição em ambientes desafiadores.</li>
                                <li><strong>Operações de Defesa:</strong> Veículo tático com alta autonomia e capacidade de adaptação.</li>
                                <li><strong>Mineração Remota:</strong> Exploração e transporte em condições inóspitas.</li>
                                <li><strong>Resgate Autônomo:</strong> Missões de busca e salvamento em áreas de difícil acesso.</li>
                            </ul>
                        `
                    },
                    {
                        id: 'cronograma',
                        subtitle: 'Cronograma e Futuro',
                        content: `
                            <p>Nosso roteiro de desenvolvimento inclui testes extensivos em campo, validação de sistemas e contínuas otimizações baseadas em dados de performance real. A Vortexa Motorsport Inc. está comprometida com a evolução da mobilidade extrema, impulsionando a inovação e a sustentabilidade.</p>
                        `
                    }
                ]
            },
            'visao-geral': {
                ...
                sections: [
                    {
                        id: "intro-institucional",
                        subtitle: "Origem e Fundação",
                        content: `
                    ...
                    {
                        id: "diferenciais",
                        subtitle: "Diferenciais Competitivos",
                        content: `
                            <ul>
                    ...
                    {
                        id: "roadmap",
                        subtitle: "Cronograma de Desenvolvimento",
                        content: `
                            <table>
                                <thead>
                                    <tr>
            ...
            'dados-tecnicos': {
                ...
                sections: [
                    {
                        id: "estrutura-design",
                        subtitle: "Arquitetura e Materiais",
                        content: `
                    ...
                    {
                        id: "especificacoes",
                        subtitle: "Dados Técnicos Detalhados",
                        content: `
                            <ul>
            ...
            'press-release': {
                ...
                sections: [
                    ...
                    {
                        id: "press-tech",
                        subtitle: "Inovações Tecnológicas",
                        content: `
                    ...
                    {
                        id: "declaracao-oficial",
                        subtitle: "Declaração Oficial de Projeto",
                        content: `
            ...
            'glossario': {
                ...
                sections: [
                    {
                        id: "termos-principais",
                        subtitle: "Termos Principais",
                        content: `
    ...
    let currentLang = localStorage.getItem('vortexaLang') || 'pt';
    ...
    function renderContent(lang) {
        ...
        if (currentContent && currentContent['sumario-executivo'] && currentContent['sumario-executivo'].sections) {
            currentContent['sumario-executivo'].sections.forEach(section => {
                ...
                mainLink.textContent = section.subtitle; // Use subtitle for main sidebar link
                sectionItem.appendChild(mainLink);
                ...
                if (section.subsections && section.subsections.length > 0) {
                    ...
                    section.subsections.forEach(sub => {
                        ...
                        subLink.textContent = sub.subtitle || sub.title; // Use subtitle or title for sub-links
        ...
        // Iterate through all top-level keys in currentContent (e.g., 'sumario-executivo', 'visao-geral', 'dados-tecnicos', 'press-release', 'glossario')
        for (const key in currentContent) {
            if (currentContent.hasOwnProperty(key) && currentContent[key].sections) {
                const mainSectionData = currentContent[key];
                
                const mainSectionElement = document.createElement('section');
                mainSectionElement.id = key; // Use the top-level key as the ID for the main section (e.g., 'sumario-executivo')
                mainSectionElement.className = 'content-section';

                const sectionTitle = document.createElement('h2');
                sectionTitle.textContent = mainSectionData.title;
                mainSectionElement.appendChild(sectionTitle);

                // Add back to top button for each main section
                const backToTopDiv = document.createElement('div');
                backToTopDiv.className = 'back-to-top';
                const backToTopButton = document.createElement('button');
                backToTopButton.className = 'back-btn';
                backToTopButton.textContent = '↑ Voltar ao Topo';
                backToTopButton.onclick = () => window.scrollTo({ top: 0, behavior: 'smooth' });
                backToTopDiv.appendChild(backToTopButton);
                mainSectionElement.appendChild(backToTopDiv);

                mainSectionData.sections.forEach(subSectionData => {
                    const subSectionElement = document.createElement('section');
                    subSectionElement.id = subSectionData.id;
                    
                    const subSectionTitle = document.createElement('h3');
                    subSectionTitle.textContent = subSectionData.subtitle || subSectionData.title;
                    subSectionElement.appendChild(subSectionTitle);

                    const sectionNav = document.createElement('div');
                    sectionNav.className = 'section-nav';
                    const navLink = document.createElement('a');
                    navLink.href = `#${key}`; // Link back to the main section
                    navLink.className = 'nav-link';
                    navLink.textContent = '← Voltar à Seção';
                    sectionNav.appendChild(navLink);
                    subSectionElement.appendChild(sectionNav);

                    const contentDiv = document.createElement('div');
                    contentDiv.innerHTML = subSectionData.content;
                    subSectionElement.appendChild(contentDiv);
                    
                    mainSectionElement.appendChild(subSectionElement);
                });
                mainContentDiv.appendChild(mainSectionElement);
            }
        }

        // After rendering content, update active links and theme
    ...
    // Função para atualizar o destaque da sidebar ao rolar
    ...
    renderContent(currentLang); // Renderiza o conteúdo inicial

    // Carrega o estado do dark mode salvo
    ...
    darkModeToggle.addEventListener('click', toggleTheme); // Usa a função toggleTheme
...
// Outras funções como scrollToTop, createReadingProgress, createSearchFunction,
// enhanceHoverEffects, addKeyboardShortcuts, setupSearch, navigateToGlossary
// etc., que existiam no seu script.js original e não foram modificadas aqui,
// devem ser mantidas se forem necessárias para a funcionalidade completa da página.
// Certifique-se de que todas as funções referenciadas no HTML (como navigateToGlossary
// no onclick dos links do glossário) estejam presentes no script.js.